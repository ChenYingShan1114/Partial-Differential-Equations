...
int main() {
  ...
  Matrix rho(N), rho_new(N), xs(N), xs_new(N), crho(N);
  ...
  int iplot = 1;
  Matrix tplot(nStep+1), xplot(N), rplot(N,nStep+1), xsplot(nStep+1), crhoplot(N,nStep+1);
  ...
  for( iStep=1; iStep<=nStep; iStep++ ) {
    ...
    for( i=1; i<=N; i++ ) { 
      rplot(i,iplot) = rho(i);
      crhoplot(i,iplot) = crho(i);
    }
  }
  int nplots = iplot;     // Number of plots recorded

  xsplot(1) = xplot(N/4);
  cout << "initial position of shock wave " << xsplot(1) << endl;
  for( j=1; j<nplots; j++ ) {
    for (i=1; i<N; i++) {
      if ( xsplot(j) > xplot(i) && xsplot(j) <= xplot(i+1)) {
        xsplot(j+1) = xsplot(j) + tau * (crhoplot(i, j) + crhoplot(i+1, j));
ax1.plot(y, phi_weight[int(0.5*y.size), :], label = 'Weighting grid', color = 'coral')
      }
    }
    if (xsplot(j+1) > xplot(N)) {
      xsplot(j+1) = xsplot(j+1) - (xplot(N) - xplot(1));
    }
  }

  //* Print out the plotting variables: tplot, xplot, rplot
  ofstream tplotOut("tplot.txt"), xplotOut("xplot.txt"), 
	      rplotOut("rplot.txt"), xsplotOut("xsplot.txt");
  for( i=1; i<=nplots; i++ ) 
    tplotOut << tplot(i) << endl;
  for( i=1; i<=nplots; i++ )
    xsplotOut << xsplot(i) << endl;
  for( i=1; i<=N; i++ ) {
