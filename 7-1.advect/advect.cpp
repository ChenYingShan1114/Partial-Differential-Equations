...
int main() {
  ...
  cout << "Choose a numerical method: 1) FTCS, 2) Lax, 3) Lax-Wendroff, 4) upwind scheme : ";
    ...
    if( method == 1 )      ////// FTCS method //////
      ...
      for( i=1; i<=N; i++ )
        a_new(i) = 0.5*( a(ip[i])+a(im[i]) ) + 
                   coeff*( a(ip[i])-a(im[i]) );   
    else if ( method == 3 ) ////// Lax-Wendroff method //////
      for( i=1; i<=N; i++ )
        a_new(i) = a(i) + coeff*( a(ip[i])-a(im[i]) ) +
                   coefflw*( a(ip[i])+a(im[i])-2*a(i) ); 
    else 
      for ( i=1;i<N; i++ )
//        cout << i << " " << ip[i]-1 << " " << im[i] << endl;
        a_new(i) = a(i) + 2*coeff*( a(ip[i]-1)-a(im[i]) );
//      cout << N << " " << N << " " << im[N] << endl;
      a_new(N) = a(N) + 2*coeff*( a(N)-a(im[N]) );
  ...
  ofstream xOut("x_test.txt"), aOut("a_test.txt"), 
	      tplotOut("tplot_test.txt"), aplotOut("aplot_test.txt");
  for( i=1; i<=N; i++ ) {
    xOut << x(i) << endl;
    aOut << a(i) << endl;
    for( j=1; j<nplots; j++ )
